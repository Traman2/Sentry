...
than an edit: pick one with `--model`, set `SENTRY_AGENT_MODEL`, or switch at runtime with
...
def resolve(name: str | None = None, *, strict: bool = False) -> ModelSpec:
    ...
    Precedence: explicit argument, then `SENTRY_AGENT_MODEL`, then the default. Accepts a
