...
from .agent import SystemExpertAgent
...
async def report_error(agent: SystemExpertAgent, space_id: int, exc: BaseException) -> None:
...
def adopt_model(agent: SystemExpertAgent, model: str | None) -> None:
...
async def answer(agent: SystemExpertAgent, space_id: int) -> None:
...
class Turns:
    ...
    def __init__(self, agent: SystemExpertAgent) -> None:
        self._agent = agent
...
async def reconcile(agent: SystemExpertAgent, turns: Turns) -> None:
...
async def handle(agent: SystemExpertAgent, turns: Turns, event: dict) -> None:
    kind = event.get("type")
...
async def serve(agent: SystemExpertAgent) -> None:
