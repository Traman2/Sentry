...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="system-expert-agent",
        description="LangGraph agent for the System-Expert desktop app's MCP tools.",
    )
...
async def run(argv: list[str] | None = None) -> int:
    ...
    try:
        agent = SystemExpertAgent(args.url or MCP_URL, model=args.model)
    ...
    try:
        ...
        try:
            ...
            print(
                f"could not reach the System-Expert MCP server at {agent.url}: {describe(exc)}",
                file=sys.stderr,
            )
