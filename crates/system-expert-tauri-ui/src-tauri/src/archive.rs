...
//! its `process_samples` rows keep ageing out at [`system_expert_core::DEFAULT_RETENTION`], so a
//! session ended without archiving shows an empty feed in the Details View a day later.

use std::path::Path;

use system_expert_core::{HistoryStore, TrackedArchive, TrackedProcess, TrackingStore};
...
pub fn end_and_archive(
    ...
    let samples = history
        .process_timeline_for_pids(&pids, system_expert_core::DEFAULT_RETENTION)
        .map_err(|e| e.to_string())?;
