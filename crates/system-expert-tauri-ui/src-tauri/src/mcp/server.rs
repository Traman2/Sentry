...
pub async fn start(
    ...
    let listener = TcpListener::bind(SocketAddr::from(([127, 0, 0, 1], PORT)))
        ...
        .inspect_err(|e| {
            if e.kind() == std::io::ErrorKind::AddrInUse {
                eprintln!(
                    "system-expert-mcp: port {PORT} is already in use — another System-Expert instance is \
    ...
    let handler = SystemExpertMcp::new(history, chat, tracking, usage, app);

    // rmcp calls this factory once per session, so the handler must be cheap to clone —
    // every field in SystemExpertMcp is an Arc or an AppHandle for exactly this reason.
    ...
    tauri::async_runtime::spawn(async move {
        ...
        // — see `SystemExpertMcp::call_tool`, which resolves it to a pid for usage logging.
        let make_service = router.into_make_service_with_connect_info::<SocketAddr>();
        if let Err(e) = axum::serve(listener, make_service).await {
            eprintln!("system-expert-mcp: server stopped: {e}");
        }
    });
    ...
    Ok(McpStatus {
        ...
        port: PORT,
        pid: std::process::id(),
        tool_count: SystemExpertMcp::tool_count(),
    })
}
