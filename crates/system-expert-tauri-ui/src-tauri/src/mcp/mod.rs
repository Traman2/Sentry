...
//! # Why in-process
//!
//! An MCP server for this app could have been its own binary linking `system-expert-core`, and
...
//! database. [`system_expert_core::Monitor`] is deliberately *not* shared — see [`SystemExpertMcp`].
...
pub struct SystemExpertMcp {
    ...
    /// [`SystemExpertMcp::call_tool`], the only place this is written.
    ...
    tool_router: ToolRouter<SystemExpertMcp>,
}
...
impl SystemExpertMcp {
    ...
    async fn log_tool_call(
        ...
        match recorded {
            ...
            Ok(Err(e)) => eprintln!("system-expert-mcp: failed to record tool-call usage: {e}"),
            Err(e) => eprintln!("system-expert-mcp: usage-logging task failed: {e}"),
        }
    }
}
...
impl ServerHandler for SystemExpertMcp {
...
mod tests {
    ...
    fn tool_count_matches_every_registered_tool_router() {
        ...
        // agent's tool list.
        assert_eq!(SystemExpertMcp::tool_count(), 22);
    }
}
