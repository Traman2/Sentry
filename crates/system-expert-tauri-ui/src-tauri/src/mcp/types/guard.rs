...
/// refused.
///
/// These live here rather than in `system-expert-core` on purpose: the core library stays a neutral
...
pub fn kill_guard(pid: u32, name: Option<&str>, force: bool) -> Option<String> {
    ...
    if pid == std::process::id() {
        return Some(format!(
            "pid {pid} is System-Expert itself — killing it would take this MCP server down with it; \
...
mod tests {
    ...
    #[test]
    fn kill_guard_refuses_system_expert_itself_unless_forced() {
