...
//!
//! Nothing here talks to `system-expert-core` — these are pure shaping functions, kept separate
//! so they stay testable without a running `Monitor` or a SQLite file.
//!
//! ## Why projections exist
//!
//! `system-expert-core`'s types are built for a UI table: every numeric field is paired with a
//! pre-formatted `_display` string, and a [`system_expert_core::SystemSnapshot`] carries every
