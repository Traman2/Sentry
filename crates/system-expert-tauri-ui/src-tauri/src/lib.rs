...
struct MonitorState(Mutex<system_expert_core::Monitor>);
...
/// `SystemExpertMcp::call_tool`, and the UI reads the same instance.
...
/// polls `get_snapshot`. See [`system_expert_core::spawn_recorder`].
const HISTORY_RECORD_INTERVAL: Duration = Duration::from_secs(2);

#[tauri::command]
fn get_snapshot(state: State<MonitorState>) -> system_expert_core::SystemSnapshot {
...
fn get_process_details(
    ...
    pid: u32,
) -> Option<system_expert_core::ProcessDetails> {
    let mut monitor = state.0.lock().unwrap();
...
/// [`system_expert_core::DEFAULT_RETENTION`].
///
...
/// and the click — see [`system_expert_core::Monitor::kill_process`].
///
...
pub fn run() {
    tauri::Builder::default()
        ...
        .manage(MonitorState(Mutex::new(system_expert_core::Monitor::new())))
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                {
                    Ok(status) => {
                        println!("system-expert-mcp: listening on {}", status.url);
                        ...
                        *state.0.lock().unwrap_or_else(|e| e.into_inner()) = Some(status);
                        ...
                        if let Err(e) = handle.state::<AgentState>().lock().start() {
                            eprintln!("system-expert-agent: not started: {e}");
                        }
                    }
                    Err(e) => eprintln!("system-expert-mcp: failed to start: {e}"),
                }
            });
