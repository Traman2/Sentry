...
fn classify(
    ...
    const RULES: &[(&str, &str)] = &[
        ...
        ("grok", "grok"),
        ("xai", "grok"),
        // The bundled agent's actual invocation (`crates/sentry-tauri-ui/src-tauri/src/agent.rs`
        ...
        ("sentry_agent", "python-agent"),
        ("sentry-agent", "python-agent"),
    ];
...
mod tests {
    ...
    fn falls_back_to_process_name_when_protocol_name_is_generic() {
        ...
        // directory — not `python -m sentry_agent`, which never appears on the command line.
        let identity = ClientIdentity {
