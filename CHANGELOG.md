...
Sentry's first stable release turns it from a system monitor into an AI-operated one: an in-app chat panel lets a language-model agent inspect and act on your machine through a full MCP tool surface, with a new usage audit trail showing exactly who called what.
...
### sentry-tauri-ui / sentry-mcp (in-process MCP server)
